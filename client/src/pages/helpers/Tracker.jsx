...
import { Button, CircularProgress, Divider, IconButton, Modal, Box } from "@mui/material";
...
const Tracker = () => {
            ...
            await Api.getExpenses({
                ...
                .then((res) => {
                    ...
                    const currentExpenses = res.data.groupedExpenses.filter(expense => {
                        return expense.month_id === currentMonth && expense.year === currentYear
                    })
    ...
    const handleDateChange = (date) => {
        ...
        const selectedExpenses = allExpenses.filter(expense => {
            return expense.month_id === selectedMonth && expense.year === selectedYear
        })
    ...
    const chartData = expenses ? expenses.expenses?.map(expense => ({
        name: expense?.description,
        value: expense?.amount,
        color: generateRandomColor()
    })) : [];

    const barChartData = expenses ? expenses.expenses?.map(expense => ({
        name: expense?.description,
        amount: expense?.amount
    })) : [];

    const paymentMethodPieChartData = expenses?.expenses?.length > 0 ? Object.entries(expenses?.expenses?.reduce((acc, { payment_method, amount }) => {
        acc[payment_method] = (acc[payment_method] || 0) + amount;
        return acc;
    }, {})).map(([name, value]) => ({ name, value, color: generateRandomColor() })) : [];

    const mostSpentDay = expenses ? expenses.expenses?.reduce((prev, current) => (prev.amount > current.amount) ? prev : current) : null;
        ...
        <Center>
            {loading ? (
                ...
                <div className="w-full flex items-start justify-start gap-8 min-h-[80vh] h-full p-4">
                    {open && <AddExpenseModal open={open} handleClose={handleClose} user={user} setChange={setChange} change={change} />}
                    ...
                    <div className='w-full flex flex-col items-start justify-start gap-8 p-4 h-[80vh] overflow-y-scroll'>
                        <div className='w-full flex items-center justify-between'>
                            <p className='font-medium text-lg'>
                                Total Expenses of the month {getSelectedMonth(selectedDate)} - <span className='font-bold underline text-xl'>₹{expenses ? expenses.totalAmount : 0}</span>
                            </p>
                            {expenses && (<Button
                                variant='contained'
                                color='success'
                        ...
                        <div className='flex flex-wrap items-start justify-start gap-8 w-full'>
                            {expenses ? (
                                expenses.expenses.map((expense, index) => (
                                    <div key={index} className='bg-white relative flex flex-col gap-2 p-4 border border-gray-200 rounded-lg shadow-lg !min-w-[30%] w-auto'>
                                        <div className={`absolute top-[-15px] right-0 rounded-full border border-gray-500 p-2 text-xs font-semibold ${expense.payment_method === 'Cash' ? 'bg-green-500' : expense.payment_method === 'Online Transfer' ? 'bg-yellow-500' : expense.payment_method === 'UPI' ? 'bg-purple-500' : expense.payment_method === 'Debit Card' ? 'bg-orange-500' : 'bg-blue-500'}`}>
                                            <span>{expense.payment_method}</span>
                                        </div>
                                        <div className='w-full flex items-center justify-between gap-2'>
                                            <div>
                                                <h1 className='text-lg font-bold'>{expense.description}</h1>
                                                <p className='text-xs font-semibold text-gray-500'>Paid to - {expense.paid_to}</p>
                                            </div>
                                            <IconButton>
                                                <InfoOutlinedIcon />
                                            </IconButton>
                                        ...
                                        <div className='w-full flex items-center justify-between gap-2'>
                                            <div>
                                                ...
                                                <p className='text-xs font-semibold text-gray-500'>{formatDate(expense.date)}</p>
                                            </div>
                                            <IconButton onClick={() => {
                                ...
                                <Center><h1>No Expenses</h1></Center>
                            )}
                        </div>
                    </div>
                    {/* <Modal
                    ...
                    <Dialog open={isVisualizeOpen} onClose={handleVisualizeClose} maxWidth="lg" fullWidth={true}>
                        <DialogTitle>Expense Breakdown</DialogTitle>
                        <DialogContent
                            className='flex items-start justify-start gap-4 w-full flex-wrap'
                        >
                            <ResponsiveContainer width="100%" height={300} >
